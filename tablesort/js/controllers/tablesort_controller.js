...
application.register('tablesort', class extends Stimulus.Controller {
	...
	_hasSortStyles() {
		let thWith = this.element.querySelector("th[aria-sort]");
		let thWithout = document.querySelector("th:not([aria-sort])");
		if ( !thWith ) return false; // No element found
	
		let oStyleWith = window.getComputedStyle(thWith);
		let oStyleWithout = window.getComputedStyle(thWithout);
	
		// Compare all computed styles
		for ( let prop of oStyleWith ) {
			if ( oStyleWith.getPropertyValue(prop) !== oStyleWithout.getPropertyValue(prop) ) {
				return true;
			}
		}
